...
const Testimonials = () => {
  ...
  return (
    <section id="testimonials" className={`w-full ${isDark ? 'bg-[#0F0F0E]' : 'bg-[#FAF9F6]'} py-16 md:py-20 lg:py-24 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          {/* Subtitle */}
          <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} font-sans text-sm md:text-base`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-12 md:mb-16">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className={`rounded-lg p-6 md:p-8 flex flex-col ${isDark ? 'bg-[#1A1A19]' : 'bg-white'} border ${isDark ? 'border-[#8B6F47]/30' : 'border-gray-200'}`}
            >
              {/* Star Rating */}
              ...
              {/* Quote */}
              <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} font-sans text-sm md:text-base leading-relaxed mb-6 flex-grow`}>
                "{testimonial.quote}"
              </p>

              {/* Reviewer Info */}
              <div className="mt-auto">
                <div className={`${isDark ? 'text-white' : 'text-gray-900'} font-sans font-medium text-sm md:text-base mb-2`}>
                  {testimonial.reviewer}
                </div>
                <div className="flex items-center gap-2">
                  <GoogleIcon />
                  <span className={`${isDark ? 'text-gray-400' : 'text-gray-600'} font-sans text-xs md:text-sm`}>
